...
官方文档给出的核心意思是：如果你需要机器稳定消费模型输出，最好使用 Structured Outputs，而不只是普通 JSON mode。JSON mode 只能提高“像 JSON” 的概率，但不能严格保证字段结构符合你的 schema。Structured Outputs 可以让模型按定义好的结构返回数据，并通过更严格的约束减少解析失败和下游异常。文档还强调 schema 设计本身很重要，字段命名、描述和约束会直接影响输出质量，因此应把 schema 纳入评测和迭代流程。
